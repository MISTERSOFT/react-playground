...
import { µAuthLogoutRequest } from '../../redux/actions/auth.actions';
import { Button } from '../ui/Button';
import { selectCartCount } from '../../redux/selectors/store.selectors';
import { µShoppingCartDrawerOpen } from '../../redux/actions/features/shopping-cart-drawer.actions';
...
export function NavMenu() {
    ...
    function logout() {
        dispatch(µAuthLogoutRequest())
    }

    function openShoppingCartDrawer() {
        dispatch(µShoppingCartDrawerOpen())
    }
        ...
        <nav className="bg-white border-gray-200 dark:bg-gray-900">
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <div className='flex items-center'>
                    ...
                    {
                        loggedUser && <Button styleType='darkTransparent' className='ml-4 relative' onClick={openShoppingCartDrawer}>
                                <CartIcon size={4} className='text-white' />
                                {
                                    cartCount > 0 ?
                                    <span className="absolute inset-y-0 inline-flex items-center justify-center w-4 h-4 text-xs font-semibold text-white bg-blue-700 rounded-full">
                                        {cartCount}
                                    </span>
                                    : ''
                                }
        ...
        <Menu as="div" className={`relative ${className}`}>
            <div>
                ...
                ">
                    <img
                        ...
                        alt="random photography"
                    />
                    {loggedUser?.name?.firstname} {loggedUser?.name?.lastname}
                    <ChevronDownIcon size={3} className="text-white" aria-hidden="true" />
                </Menu.Button>
            </div>
            ...
            >
                <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="py-1">
                        <Menu.Item>
                            <Link to='/' className='text-gray-700 block px-4 py-2 text-sm'>Account settings</Link>
                        </Menu.Item>
                        <Menu.Item>
                            <button
                                type="button"
